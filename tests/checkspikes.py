...
from dissonance import epochtypes as et
from dissonance.funks.passfilters import high_pass_filter, low_pass_filter

import plotly.express as px

df = pd.read_csv("/home/joe/Projects/Discordance/PlotTrace_2022-04-25 18:09:31.626546.csv")
...
def plot(A):
    df = pd.DataFrame(columns="Time Value".split())
    df["Value"] = A
    df["Time"] = np.arange(len(A))
...
# PASS FILTERS
# REMOVE HIGH FREQUENCIES TO GET NOISE OF UNDERLYING
...
len(peak_times_res_even)
peak_times_res_odd

peak_times_res_even = 2 * peak_times_res_even
